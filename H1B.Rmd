...
This dataset is from the website Kaggle.com that being said the data may not be 100 percent correct. Reading the forums on the kaggle website one field I and other found challenging is what does certified- withdrawn, withdrawn , certified and certified withdrawn mean? A user of the forum posted this:    
...
```   
87.1 percent of applicants got a certified status from 2011 to 2016.   
...
ggplot(top_employers, aes(x = reorder(EMPLOYER_NAME, percent),
                      ...
                      labs(x = "Employer Name", y = "Number of jobs") +  
                      theme(legend.position = "none") +
                      coord_flip()
```   
...
Lets take a look at where the most H1Bs are working. 
```{r, message=FALSE, warning=FALSE, echo=FALSE}
...
```    
We can see that most positions are full time. Because the data set is from 2011 to 2016 I can see if this has changed over the years.   
